...
namespace AirlineReservation
{
    ...
    {
        //need passenger and seat objects
        Passenger passenger;
        Seat seat;

        //list of all seat (used to display the seats in the list box)
        ...
        //when the form loads, display the seats and populate the drop down with seat rows.
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            //check the column that was selected (A, B, C, D)
            var checkedButton = gbColumn.Controls.OfType<RadioButton>().FirstOrDefault(r => r.Checked);

            // validate input - name, seat and column selection are required
            ...
            if (seat.IsPlaneFull())
            {
                var msg = MessageBox.Show("The plane is full.  Add passenger to waiting list?", "Plane Full", MessageBoxButtons.YesNo,
                ...
                using(var conn = new OleDbConnection(DatabaseObjects.ConnectionString))
                {
                    ...
                    MessageBox.Show("Passenger " + txtName.Text + " was added to the waiting list", "Waiting List", MessageBoxButtons.OK,
                        MessageBoxIcon.Information);
            ...
            //check if seat istaken. If it is, exit so the user can select a different seat
            if (seat.IsSeateAlreadyTaken(cmbSeatRow.SelectedItem.ToString(), checkedButton.Text))
            {
                MessageBox.Show("The seat is already taken.  Please select a different seat.", "Seat Taken", MessageBoxButtons.OK,
                    MessageBoxIcon.Information);

                return;
            }
            //if everything is OK, add passenger and seat to database along with the assinged seat
            using (var conn = new OleDbConnection(DatabaseObjects.ConnectionString))
            {
                conn.Open();
                ...
                MessageBox.Show("Passenger was added.", "Added Passenger", MessageBoxButtons.OK,
                    MessageBoxIcon.Information);
        ...
        //create a method that displays the seats on the list box
        private void PopulateAirplane()
        {
            //clear previous listbox and list of seats
            lstOutput.Items.Clear();
            seats.Clear();
            //select * from Seats db. Read result and create a Seat object with
            ...
            using (var conn = new OleDbConnection(DatabaseObjects.ConnectionString))
            {
                ...
                while (reader.Read())
                {
                    ...
                    seat.SeatColumn = reader["SeatColumn"].ToString(); //there is an error when using instructor's code.
        ...
        //populate combo box with seat rows
